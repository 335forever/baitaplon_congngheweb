...
export default function ProductDetail({ category, categoryID, name, description, images, price, productID, quantity, shoperID, onCancel, onUpdate }) {
  ...
  const [_quantity, setQuantity] = React.useState();
  const [_categoryId, setCategoryId] = React.useState();
  ...
  function verify() {
    var imageList = [];
    var refImages = Object.values(images)

    const formData = new FormData()

    for (let i = 0; i < 5; i++) {
      if (imagePaths[i] == refImages[i] && imagePaths[i] != null) {
        imageList.push(imagePaths[i]);
      } else {
    ...
    uploadImages(formData, () => { },
      (res) => {
        imageList = [...imageList, ...Object.values(res.data.images)]
  ...
  useEffect(() => {
    ...
    setImagePaths(Object.values(images == null ? [] : images))
    setPrice(price)
    setCategoryId(categoryID)
  }, [name, description, quantity, price, categoryID, images]);
